...
- All new database tables must be created using SQLC and the corresponding queries must be added to the `/pkg/core/db/sqlc` directory.
- All new database types must be added to the `/pkg/core/db/types.go` file./modules/{module}/backend/migrations` directory
- Once the migration and SQLC files are added, run `make sqlc` to generate the database code.
- Handlers are located in the `/pkg/core/api` directory.
- Services are located in the `/pkg/core/service` directory.
- Database operations are located in the `/pkg/core/db` directory.
