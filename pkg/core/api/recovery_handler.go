...
// SetPassword sets the user's password after recovery
// POST /public-api/v1/auth/set-password
func (h *RecoveryHandler) SetPassword(c *gin.Context) {
	var req struct {
	...
	log.Info().
		Str("flow_id", req.FlowID).
		Str("csrf_token_length", fmt.Sprintf("%d", len(req.CSRFToken))).
		Msg("Setting password via direct Kratos call")
	...
	// Create HTTP request to Kratos
	kratosURL := fmt.Sprintf("%s/self-service/settings?flow=%s", h.kratosPublicURL, req.FlowID)
	httpReq, err := http.NewRequest("POST", kratosURL, bytes.NewReader(bodyBytes))
	if err != nil {
	...
	if cookieHeader != "" {
		...
		log.Info().
			Str("cookie_header_preview", cookieHeader[:min(len(cookieHeader), 100)]+"...").
			Strs("cookie_names", cookieNames).
			Int("cookie_count", len(cookies)).
			Msg("Forwarding Cookie header to Kratos")
	} else {
		log.Warn().Msg("No Cookie header found in request")
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("Failed to call Kratos")
		c.JSON(http.StatusInternalServerError, helpers.ErrorResponse(err))
		return
	}
	defer resp.Body.Close()
	...
	log.Info().
		Int("status", resp.StatusCode).
		Str("response_preview", string(respBody[:min(len(respBody), 200)])+"...").
		Msg("Kratos response received")

	// Handle response
	if resp.StatusCode == http.StatusOK {
		// Success
		var flowResponse map[string]interface{}
		if err := json.Unmarshal(respBody, &flowResponse); err != nil {
			log.Error().Err(err).Msg("Failed to parse success response")
			c.JSON(http.StatusInternalServerError, helpers.ErrorResponse(err))
			return
		}
		...
		log.Error().
			Int("status", resp.StatusCode).
			Interface("error_response", errorResponse).
			Msg("Kratos returned error")
